...
$stmt = $pdo->prepare(
    "SELECT b.*, e.title AS event_title, e.date AS event_date, e.location AS event_location,
            e.description AS event_description, e.budget AS event_budget, e.max_attendees AS event_max_attendees
       FROM bookings b
...
// Map booking/event statuses to UI statuses
...
if (!empty($dbEvents)) {
    foreach ($dbEvents as $evt) {
        $bookings[] = [
            ...
            'event_title' => $evt['event_title'] ?: 'Untitled Event',
            'event_date' => $evt['event_date'] ?: 'TBD',
            'status' => mapEventStatus($evt['status'] ?? 'pending'),
            'guest_count' => (int)($evt['guest_count'] ?? 0),
            'max_guests' => (int)($evt['event_max_attendees'] ?? 200),
            'customer_name' => ($evt['customer_name'] ?? '') ?: $userFullName,
            'email' => $userEmail,
            'phone' => ($evt['customer_contact'] ?? '') ?: $userPhone,
            'address' => ($evt['customer_address'] ?? '') ?: $userAddress,
            'description' => $evt['event_description'] ?: 'No description provided',
            'package' => ucfirst($evt['status'] ?? 'Pending') . ' Package',
            'budget' => (float)($evt['booking_budget'] ?? $evt['event_budget'] ?? 0),
            'created_at' => $evt['booking_date'] ? date('Y-m-d', strtotime($evt['booking_date'])) : date('Y-m-d'),
        ];
    }
}
...
<body class="min-h-screen flex flex-col">
                    ...
                    <div class="booking-card" id="booking-<?php echo $bk['id']; ?>">
                        ...
                        <div class="booking-header">
                            ...
                            <div class="flex items-center gap-3 shrink-0">
                                ...
                                <button type="button" class="btn-outline" onclick="viewExpenses(<?php echo $bk['id']; ?>)">View Expenses</button>
    ...
    <div id="expensesModal" class="modal-overlay">
        <div class="modal-box" style="max-width: 560px;">
            <div class="text-center mb-6">
                ...
                <p class="text-sm mt-2" style="color:#62744f;">Expenses for <strong id="expenseEventName"></strong></p>
            </div>
            <div class="bg-white rounded-xl p-4 mb-4 border" style="border-color:#e8ebe3;">
                <div class="flex justify-between items-center mb-4">
                    <span class="text-sm font-semibold" style="color:#353f2d;">Total spent</span>
                    <span class="text-sm font-bold" style="color:#1a472a;">₱<span id="expenseTotal">0.00</span></span>
                </div>
                <div id="expenseList" class="space-y-3" style="max-height:320px; overflow:auto;"></div>
            </div>
            <div class="flex gap-3">
                <button onclick="closeModal('expensesModal')" class="btn-outline flex-1">Close</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function viewExpenses(id) {
            const bk = <?php echo json_encode($bookings); ?>.find(b => b.id == id);
            if (!bk) return;

            const modal = document.getElementById('expensesModal');
            const eventNameEl = document.getElementById('expenseEventName');
            const expenseList = document.getElementById('expenseList');
            const expenseTotal = document.getElementById('expenseTotal');

            eventNameEl.textContent = bk.event_title || 'Event';
            expenseList.innerHTML = '<p class="text-sm" style="color:#62744f;">Loading expenses...</p>';
            ...
            try {
                ...
                url.searchParams.set('event_id', bk.event_id);
                ...
                if (!expenses.length) {
                    expenseList.innerHTML = '<p class="text-sm" style="color:#62744f;">No expenses have been recorded for this event yet.</p>';
                    return;
                }
                ...
                expenseList.innerHTML = expenses.map(exp => {
                    ...
                    return `
                        <div class="rounded-2xl p-4" style="background:#f4f7f2; border:1px solid #e8ebe3;">
                            <div class="flex justify-between items-center gap-3">
                                <span class="font-semibold text-sm" style="color:#1a472a;">${exp.category || 'Expense'}</span>
                                <span class="text-sm font-bold" style="color:#1a472a;">₱${Number(exp.amount || 0).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}</span>
                            </div>
                            <p class="text-xs mt-2" style="color:#62744f;">${exp.description || 'No description available'}</p>
                            <p class="text-xs mt-1" style="color:#8e9b8c;">${exp.expense_date || ''}</p>
                        </div>
                    `;
                }).join('');
                expenseTotal.textContent = total.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2});
            } catch (err) {
                expenseList.innerHTML = `<p class="text-sm" style="color:#b91c1c;">${err.message}</p>`;
            }
        }
