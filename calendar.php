...
// Get the user's booked events for the calendar
$stmt = $pdo->prepare("
    SELECT b.*, e.title AS event_title, e.description, e.max_attendees
    FROM bookings b
...
if (!empty($events)) {
    foreach ($events as $evt) {
        $eventDate = $evt['event_date'] ?? $evt['date'] ?? '';
        $eventTitle = $evt['event_title'] ?? $evt['title'] ?? 'Untitled Event';
        ...
        $calendarEvents[] = [
            ...
            'date' => $eventDate,
            'customer' => $userName,
            'attendees' => (int)($evt['guest_count'] ?? 0),
            'max_attendees' => (int)($evt['max_attendees'] ?? 200),
            'description' => $evt['description'] ?? '',
            'status' => $evt['status'] ?? 'pending',
            'color' => getEventColor($evt['status'] ?? 'pending', $eventDate, $today)
        ];
    }
}
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .card {
            background: #ffffff;
            border-radius: 20px;
            box-shadow: 0 12px 30px rgba(37, 51, 38, 0.08);
            transition: all 0.2s ease;
        }
        .card:hover { box-shadow: 0 14px 40px rgba(37, 51, 38, 0.12); }
        .divider { border-top: 1px solid #e7ece3; margin: 1.5rem 0; }
        ...
        .event-pill {
            display: block;
            padding: 8px 10px;
            background: rgba(223, 242, 242, 0.9);
            border-radius: 9999px;
            margin-bottom: 8px;
            font-size: 0.75rem;
            font-weight: 600;
        ...
        .event-pill:hover {
            box-shadow: 0 8px 18px rgba(37, 51, 38, 0.08);
        }
        ...
        /* Pill popover click to view */
        .event-pill { text-decoration: none; }
    </style>
</head>
    ...
    <main class="pt-24 pb-8 px-6 max-w-6xl mx-auto">
            ...
            <div class="mb-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h2 class="text-2xl font-bold" style="color:#1a472a;">Event Calendar</h2>
                        <p class="text-sm mt-1" style="color:#62744f;">Browse and filter events by month</p>
                    </div>
                </div>
            </div>
            ...
            <div id="calendar-section" class="card p-6 mb-6">
                ...
                <div class="text-center py-10 mt-4" style="border-top:1px solid #e8ebe3;">
                    <i class="fas fa-calendar-xmark text-3xl mb-3" style="color:#c5d8bf;"></i>
                    <p class="text-sm font-medium" style="color:#62744f;">No events scheduled</p>
                    <p class="text-xs mt-1" style="color:#a3b095;">Add events from the Event Planner page to see them here.</p>
                </div>
                <?php endif; ?>
            </div>
