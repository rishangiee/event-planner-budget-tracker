...
// Login page accessible for all users (no auto-redirect)
...
if ($_POST) {
    ...
    if ($user && verifyPassword($password, $user['password'])) {
        ...
        $redirectAfterModal = $redirectUrl . ($hasPendingBooking ? '?book=1' : '');
        $_SESSION['login_success'] = [
            'role' => $userRole,
            'redirect' => $redirectAfterModal,
            'message' => $hasPendingBooking ? 'Your booking is being processed!' : 'Welcome back! Let\'s get started.'
        ];
    } else {
...
<script>
        ...
        // Handle pending booking from sessionStorage on load
        window.addEventListener('load', function() {
            ...
            if (pendingData) {
                ...
                sessionStorage.removeItem('pending_booking'); // Clear after passing
            }
        });
